...
// it generally makes sense to run some simple reality checks on 
// the number. The numbers are a good length and it's common to 
// make minor transcription errors when the card is not scanned 
// directly.
...
// - Starting with the first digit and continuing with every other 
//   digit, multiply by 2
...
function isValid(ccnum){
	...
	var tog = ccnum.split('');
	tog = tog.map(function(ch){
		return parseInt(ch, 10);
	})
	console.log(tog);

	for (var i = 1; i < tog.length; i += 2 ){
		tog[i] = tog[i]*2;
		}

	console.log(tog);

	var total = tog.reduce(function(a,b){
		return a+b;
	});
	return total % 10 === 0;
};
